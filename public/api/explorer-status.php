...
const EXPLORER_PACKAGE_CANDIDATES = [
    ...
    '/opt/btc-rpc-explorer/package.json',
    '/srv/btc-rpc-explorer/package.json',
    '/var/www/btc-rpc-explorer/package.json',
];
...
function get_explorer_service_status(): array {
    $systemctlResponded = false;

    foreach (EXPLORER_SERVICE_CANDIDATES as $serviceName) {
...
function get_explorer_version(): ?string {
    foreach (EXPLORER_PACKAGE_CANDIDATES as $packageFile) {
...
function explorer_http_available(): bool {
    $socket = @stream_socket_client(
        'tcp://127.0.0.1:' . dashboard_service_port('explorer'),
        $errno,
        $errstr,
        2
    );
...
$service = get_explorer_service_status();
$version = ($service['status'] === 'not installed') ? null : get_explorer_version();
$httpAvailable = $service['status'] === 'running' ? explorer_http_available() : false;
