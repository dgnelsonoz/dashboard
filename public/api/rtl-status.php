...
const RTL_SERVICE_CANDIDATES = [
    'rtl.service',
];
const RTL_PACKAGE_CANDIDATES = [
    '/usr/local/lib/rtl/package.json',
    '/usr/local/src/rtl/package.json',
];
...
function get_rtl_service_status(): array {
    $systemctlResponded = false;

    foreach (RTL_SERVICE_CANDIDATES as $serviceName) {
...
function get_rtl_version(): ?string {
    foreach (RTL_PACKAGE_CANDIDATES as $packageFile) {
...
function rtl_http_available(): bool {
    ...
    $headers = @get_headers(dashboard_local_service_url('rtl'), false, $ctx);
...
$service = get_rtl_service_status();
$version = ($service['status'] === 'not installed') ? null : get_rtl_version();
$httpAvailable = $service['status'] === 'running' ? rtl_http_available() : false;
